...
from ..items import Director_Url_Spider_Item
from .. import db,Movie_Url
from time import  sleep

class DirectorSpider(scrapy.Spider):
    name = "douban_director"
    ...
    def parse(self, response):
        ...
        for url in url_list :
            sleep(1)
            yield Request(url.M_Uurl,cookies=self.cookies,callback=self.parse_director)

    def parse_director(self, response):
        item = Director_Url_Spider_Item()

        for sel in response.xpath('//div[@id="info"]/span[1]/span[@class="attrs"]'):
            item['director_name'] = sel.xpath('./a/text()').extract()[0]
            item['director_url'] = sel.xpath('./a/@href').extract()[0]
            yield item
