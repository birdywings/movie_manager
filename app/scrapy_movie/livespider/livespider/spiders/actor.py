...
from ..items import Actor_Url_Spider_Item
...
class ActorSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for url in url_list :
            sleep(1)
            yield Request(url.M_Uurl,cookies=self.cookies,callback=self.parse_actor)

    def parse_actor(self, response):
        item = Actor_Url_Spider_Item()
        for value in [0,1]:
            for sel in response.xpath('//div[@id="info"]/span[@class="actor"]/span[@class="attrs"]'):
                item['actor_name'] = sel.xpath('./a/text()').extract()[value]
                item['actor_url'] = sel.xpath('./a/@href').extract()[value]
                yield item
