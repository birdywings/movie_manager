...
from ..items import Actor_Item
from .. import db,Actor_Url
from time import  sleep

class A250actorSpider(scrapy.Spider):
    name = "250actor"
    ...
    def parse(self, response):
        url_list = Actor_Url.query.all()

        for url in url_list[200:] :
            sleep(1)
            yield Request('https://movie.douban.com' + url.A_Uurl, cookies=self.cookies, callback=self.parse_actor)
    ...
    def parse_actor(self, response):
        ...
        if len(response.xpath('//*[@id="headline"]/div[2]/ul/li[4]/text()').extract()) < 2 :
            ...
            item['Acome_from'] = response.xpath('//*[@id="headline"]/div[2]/ul/li[4]/text()').extract()[1].strip(':').strip()
