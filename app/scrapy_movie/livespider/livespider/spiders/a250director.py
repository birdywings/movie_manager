...
from ..items import Director_Item
from .. import db,Director_Url
from time import  sleep


class A250directorSpider(scrapy.Spider):
    name = "250director"
    ...
    def parse(self, response):
        ...
        url_list = Director_Url.query.all()

        for url in url_list[0:1] :
            sleep(1)
            yield Request('https://movie.douban.com'+url.D_Uurl, cookies=self.cookies, callback=self.parse_director)
    ...
    def parse_director(self, response):
        ...
        if len(response.xpath('//*[@id="headline"]/div[2]/ul/li[4]/text()').extract()) < 2 :
            ...
            item['Dcome_from'] = response.xpath('//*[@id="headline"]/div[2]/ul/li[4]/text()').extract()[1].strip(':').strip()
